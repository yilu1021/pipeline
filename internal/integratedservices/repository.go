...
func (c clusterRepository) transform(instance v1alpha1.ServiceInstance) IntegratedService {
	...
	// todo config json to be transformed: string -> json
	cfgJSON := make(map[string]interface{})
	if err := json.Unmarshal([]byte(instance.Spec.Config), &cfgJSON); err != nil {
		println("valami")
	}

	mapstructure.Decode(cfgJSON, &transformedIS.Spec)
